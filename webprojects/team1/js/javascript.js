...
function main() {
    ...
    if (regForm) {
        regForm.addEventListener('submit', function (event) {
            ...
            var inputFields = document.querySelectorAll('#regForm input[type="text"]');//.getElementsByClassName('required');
        ...
        regForm.addEventListener('reset', function (event) {
            var inputFields = querySelectorAll('#regForm input[type]:not([type="submit"])');
            if (inputFields) {
                for (var i = 0; i < inputFields.length; i++) {
